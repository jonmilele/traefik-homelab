...
graph LR
    ...
    subgraph traefik_host["Traefik Host — 192.0.2.10"]
        ...
        sync["dns-sync<br/>(systemd daemon)"]
        redis[("Redis")]
    end
...
### [dns-sync/](dns-sync/)
A custom Python script that automatically syncs Traefik router hostnames to Pi-hole v6 local DNS. Runs as a systemd daemon (configurable interval, default 120s), with change detection, backup/rollback, and per-instance Pi-hole authentication. Includes a `traefik-pihole-sync.service` systemd unit file. See the [traefik-pihole-sync](https://github.com/jonnewman85/traefik-pihole-sync) repo for the standalone version.
...
### DNS
No action needed — the dns-sync daemon will automatically detect the new hostname from the Traefik API and add it to both Pi-hole instances on the next sync cycle.
