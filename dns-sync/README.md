...
┌─────────────┐     poll routers     ┌───────────────┐
│  Traefik    │◄─────────────────────│  sync.py      │
│  API :8080  │   every 120s (cfg)   │  (daemon)     │
└─────────────┘                      └───────┬───────┘
                                             │
                              compare hash   │
                              with cache     │
...
- **Daemon mode** — Runs as a long-lived systemd service with configurable poll interval
...
sudo cp sync.py sync.sh traefik-pihole-sync.service /opt/traefik-dns-sync/
